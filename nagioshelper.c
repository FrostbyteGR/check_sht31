...
#define THRNG_DISABLE_MIN -130
#define THRNG_DISABLE_MAX 130
...
static void throwError(int errorCode) {
	...
	switch(errorCode) {
		case ERRCODE_USAGE:
			...
			"Additional commands may be issued with the -x argument:\n" \
			"getEnvData, getSerial, getStatus, clearStatus, softReset, heaterEnable, heaterDisable\n");
			break;
		case ERRCODE_INVALID_BUS:
			fprintf(stderr, "Invalid I2C bus specified, must be a number.\n");
			break;
...
static struct execParameters defaultParameters() {
	...
	defaults.bus="/dev/null";
	defaults.cmd="getEnvData";
...
// Parser function for user input: I2C bus
static char* parseI2CBus(char *inputString) {
	// Convert input to a device file path
	char *result=(char*)malloc(sizeof(inputString)+9);
	sprintf(result, "/dev/i2c-%d", atoi(inputString));

	// Check input for any non-numerical characters
	while (*inputString) {
		if (isdigit(*inputString++)==0) {
			// If a non-numerical character is found, throw the corresponding error
			throwError(ERRCODE_INVALID_BUS);
		}
	}

	// Return the processed device file path
	return result;
}
...
struct execParameters parseParameters(int argc, char *argv[]) {
	...
	while ((argument=getopt(argc, argv, "b:x:w:c:"))!=-1) {
		switch(argument) {
			case 'b':
				result.bus=parseI2CBus(optarg);
				break;
			case 'x':
				result.cmd=optarg;
				break;
	...
	// If the user did not supply an I2C bus
	if (strcasecmp(result.bus, "/dev/null")==0) {
		// Respond with the usage error
		throwError(ERRCODE_USAGE);
	}

	// If environment data were requested
	if (strcasecmp(result.cmd, "getEnvData")==0) {
